 ...
 * - تم نقل ملفات البيانات إلى: src/data/
 * - تم نقل الصور إلى: public/images/
 * - يُرجى عند إضافة ملف جديد الالتزام بهذه البنية.
 */
/* 
  موقع عربي بستايل قريب من Massively
  - كل المحتوى يأتي من ملفات JSON داخل src/data/
  - الصور داخل public/images/ (ثوابت نسب عبر CSS)
...
async function applyUIText() {
  const uiData = await loadData("ui.json");

  // تحديث البيانات الشخصية
  const profileName = document.getElementById("profile-name");
  if (profileName) profileName.textContent = uiData.profile.name;

  const profileRole = document.getElementById("profile-role");
  if (profileRole) profileRole.textContent = uiData.profile.role;

  // تحديث عناصر النافبار
  const navIds = ["home", "about", "skills", "projects", "articles", "contact"];
  navIds.forEach(id => {
    const el = document.getElementById(`nav-${id}`);
    if (el) el.textContent = uiData.nav[id];
  });

  // تحديث الفوتر
  const profileNameFoot = document.getElementById("profile-name-foot");
  if (profileNameFoot) profileNameFoot.textContent = uiData.profile.name;

  const footerRights = document.querySelector(".site-footer small");
  if (footerRights) {
    footerRights.lastChild.textContent = ` ${uiData.footer.rights}`;
  }
}

// تنفيذ بعد تحميل الصفحة
document.addEventListener("DOMContentLoaded", applyUIText);
...
async function loadAll(){
  ...
  const [profile, about, skills, projects, articles] = await Promise.all([
    loadJSON('src/data/profile.json'),
    loadJSON('src/data/about.json'),
    loadJSON('src/data/skills.json'),
    loadJSON('src/data/projects.json'),
    loadJSON('src/data/articles.json')
  ]);
...
function renderHome(){
  ...
  app.innerHTML = `
    <!-- HERO بخلفية من public/images/profile/hero.jpg -->
...
function renderAbout(){
  ...
  const profileImg = 'public/images/profile/profile.jpg'; // ضع صورتك هنا
